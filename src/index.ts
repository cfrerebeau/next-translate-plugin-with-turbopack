...
function nextTranslate(nextConfig: NextConfig = {}): NextConfig {
  ...
  let nextConfigWithI18n: NextConfig = hasAppJs
    ...
    // @ts-ignore
  const isUsingTurbopack = nextConfig.experimental && nextConfig.experimental.turbo

  if (isUsingTurbopack) {
    // Turbopack configuration using SWC plugins
    return {
      ...nextConfigWithI18n,
      experimental: {
        ...
        swcPlugins: [
          ...
          [
            require.resolve('next-translate-plugin/swc-plugin'),
            {
              basePath,
